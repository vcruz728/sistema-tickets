// resources/js/Components/TicketCardSoporte.tsx
...
interface Props {
  ticket: Ticket;
  onClick?: (ticket: Ticket) => void; // opcional, por si prefieres modal
}

const TicketCardSoporte: React.FC<Props> = ({ ticket, onClick }) => {
  const handleClick = () => {
    if (onClick) {
      onClick(ticket); // abre modal si se pasa
    } else {
      Inertia.visit(`/soporte/responder/${ticket.id}`); // redirige por defecto
    }
  };
...
export default TicketCardSoporte;
